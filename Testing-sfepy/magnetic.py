...
def mesh_hook(mesh, mode):
    ...
    """
    if mode == 'read':
        mesh = gen_block_mesh(dims, shape, [0, 0], name='magnetic_block',
                              verbose=False)
        return mesh
...
fields = { # 
    'mag flux B': ('real', 'vector', 'Omega', 2),
    'mag field H': ('real', 'vector', 'Omega', 2),
    'magnetization M': ('real', 'vector', 'Omega', 2)
}
...
variables = {
    'B' : ('unknown field', 'mag flux B', 0),
    'bb': ('test field', 'mag flux B', 'B'),
    'H' : ('unknown field', 'mag field H', 1),
    'hh': ('test field', 'mag field H', 'H'),
    'm': ('parameter field', 'magnetization M', 0)
}
